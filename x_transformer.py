...
class DynamicPositionBias(nn.Module):
    def __init__(self, dim, *, heads, depth, log_distance = False, norm = False, num_mem=1, mask_for_mem=True):
        super().__init__()
        assert depth >= 1, 'depth for dynamic position bias MLP must be greater or equal to 1'
        self.log_distance = log_distance
        self.num_mem = num_mem
        self.mlp = nn.ModuleList([])
        self.mask_for_mem = mask_for_mem
    ...
    def forward(self, i, j):
        assert i == j        
        n, device = j - self.num_mem, self.device
        ...
        bias = F.pad(bias, (self.num_mem,0,self.num_mem, 0), value = 0.)
        return bias
    
def pairwise_diff_reminder(ints1, ints2, intervals, device):
    if len(ints1.shape) == 2:
        funcs = ['b i -> b i 1 1', 'b j -> b 1 j 1', 'l -> 1 1 1 l']
    elif len(ints1.shape) == 1:
        funcs = ['i -> i 1 1', 'j -> 1 j 1', 'l -> 1 1 l']
    else:
        raise NotImplementedError(len(ints1.shape))
            
    int_diff = rearrange(ints1, funcs[0]) - rearrange(ints2, funcs[1])
    intervals = rearrange(intervals.to(device), funcs[2])
    int_diffs_reminds = (int_diff % intervals).to(float) / intervals
    return int_diffs_reminds

class DynamicPositionTimeBias(nn.Module):
    def __init__(self, dim, *, heads, depth, time_intervals, 
            max_pos=512, pos_intervals_num=1,
            use_segm_diff=False, use_segm=False,
            log_distance = False, norm = False, 
            num_mem=1, mask_for_mem=True):
        super().__init__()
        assert depth >= 1, 'depth for dynamic position bias MLP must be greater or equal to 1'
        self.log_distance = log_distance
        self.num_mem = num_mem
        ...
        self.use_segm_diff = use_segm_diff
        self.pos_intervals_num = pos_intervals_num

        self.pos_intervals = torch.from_numpy(
            np.array([
                pow(self.max_pos, (1 - i / float(self.pos_intervals_num)))
                for i in range(self.pos_intervals_num)]))

        self.mlp.append(Sequential(
            nn.Linear(
                self.pos_intervals_num + len(self.time_intervals) \
                + int(use_segm_diff) + int(use_segm), dim),
            LayerNorm(dim) if norm else None,
            nn.SiLU()
        ))
    ...
    def forward(self, i, j, time_i, time_j, segm_i=None, segm_j=None):
        assert i == j        
        n, device = j - self.num_mem, self.device
        # get the (n x n) matrix of distances
        seq_arange = torch.arange(n, device = device)
        context_arange = torch.arange(n, device = device)
        pos_diff = pairwise_diff_reminder(
            seq_arange, context_arange, self.pos_intervals, self.device)
        pos_diff_batch = repeat(pos_diff, "i j l -> b i j l", b=time_i.shape[0])

        time_diff = pairwise_diff_reminder(
            time_i, time_j, self.time_intervals, self.device)
        
        arrs_to_concat = [pos_diff_batch, time_diff]
        if self.use_segm_diff and not(segm_i is None):
            segm_diff = rearrange(segm_i.to(self.device), 'b i -> b i 1 1') - rearrange(segm_j.to(self.device), 'b j -> b 1 j 1')
            diffs = arrs_to_concat.append(segm_diff)

            if self.use_segm:
                segm = repeat((2.0 * segm_i - 1.0).to(self.device), 
                    'b i -> b i j 1', j=segm_i.shape[1]) 
                arrs_to_concat.append(segm)

        diffs = torch.concat(arrs_to_concat, dim=3)
        diffs_for_mlp = rearrange(diffs, "b i j l -> (b i j) l").float() #.to(self.mlp.dtype)
        #print(diffs_for_mlp)
        for layer in self.mlp:
            diffs_for_mlp = layer(diffs_for_mlp)

        # get position biases        
        #bias = pos[indices]
        bias = rearrange(diffs_for_mlp, '(b i j) h -> b h i j', 
                         b=diffs.shape[0], i=diffs.shape[1], j=diffs.shape[2])
        bias = F.pad(bias, (self.num_mem,0,self.num_mem, 0), value = 0.)
        return bias
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        attn_bias = None
        if exists(rel_pos):
            if type(rel_pos) is DynamicPositionTimeBias:
                attn_bias = rel_pos(i, j, time, time, segm_i=segm, segm_j=segm)
            else:
...
class AttentionLayers(nn.Module):
    def __init__(
        ...
        dynamic_pos_bias_use_time = False,
        dynamic_pos_bias_use_segm = False,
        dynamic_pos_bias_use_segm_diff = False,
        ...
        **kwargs
        ...
        if rel_pos_bias:
            ...
            num_mem = kwargs.get("num_mem", 1)
            mask_for_mem = kwargs.get("mask_for_mem", True)
            
            if dynamic_pos_bias_use_time:
                hour_len = 60 * 60 #* 1000 #in ms
                day_len = 24 * hour_len
                time_intervals=np.array([365 // 4 * day_len, 30 * day_len, 7 * day_len, day_len])
                self.rel_pos = DynamicPositionTimeBias(dim = dim // 4, heads = heads, 
                                time_intervals=time_intervals,
                                use_segm_diff=dynamic_pos_bias_use_segm_diff,
                                use_segm=dynamic_pos_bias_use_segm,
                ...
                self.rel_pos = DynamicPositionBias(dim = dim // 4, heads = heads, 
                                                   log_distance = dynamic_pos_bias_log_distance, 
                                                   depth = dynamic_pos_bias_mlp_depth, 
                                                   norm = dynamic_pos_bias_norm, 
                                                   num_mem=num_mem, 
        ...
        for ind, (layer_type, layer_shift_tokens) in enumerate(zip(self.layer_types, shift_tokens)):
            ...
            if layer_type == 'a':
                ...
                layer = Attention(kwargs['i_dim'] + 1, heads = heads, **attn_kwargs)
            elif layer_type == 'c':
    ...
    def forward(
        ...
        for ind, (layer_type, (norm, block, residual_fn), layer_dropout) in enumerate(zip(*layer_variables)):
            ...
            if layer_type == 'a':
                ...
                return_intermediates = True, time=time, segm=segm)
            elif layer_type == 'c':
                ...
                to_pass = rearrange(x, "b l w -> b w l") 
                out = block(to_pass)
                out = rearrange(out, "b w l -> b l w") 
            if self.resi_dual:
...
class XTransformerEncoder(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        attn_layers = None,
        num_memory_tokens = 1,
        reduce_type="first_mem",#options: "last_token", "mean", "mean_mem", "all_mem"
        post_emb_norm = False,
        is_reduce_sequence = True,
        use_pos_embs=False,
        use_time2vec=False,
        different_time_embs=False,
        **kwargs
    ):
        super().__init__()
        ...
        self.time_embs = None
        self.pos_embs = None
        self.different_time_embs = different_time_embs
        ...
        if use_time2vec:
            ...
            if different_time_embs:
                self.time_embs = nn.ModuleList([
                    Time2Vec(time_intervals, emb_dim),
                    Time2Vec(time_intervals, emb_dim)])
            else:
                self.time_embs = Time2Vec(time_intervals, emb_dim)

        if use_pos_embs:
            self.pos_embs = ScaledSinusoidalEmbedding(emb_dim)
    ...
    def forward(
        ...
        **kwargs
        ...
        if not(self.pos_embs is None):
            pos_embs = self.pos_embs(x)
            x += pos_embs

        if not (time is None):
            time[time>9999999999] //= 1000
            time[time>9999999999] //= 1000

        if not(self.time_embs is None):
            if self.different_time_embs:
                segm_vec = rearrange(segm, 'b l -> b l 1')
                time_embs = self.time_embs[0](time) * (1 - segm_vec) + \
                    self.time_embs[1](time) * segm_vec.clone()
            else:
                time_embs = self.time_embs(time)
        ...
        x = self.attn_layers(x, mask = mask, time = time, segm=segm, **kwargs)
        x = self.norm(x)

        mem, x = x[:, :num_mem], x[:, num_mem:]
        if self.is_reduce_sequence:
            if self.reduce_type == "last_token":
                return x[:,-1,:]
            elif self.reduce_type == "first_mem":
                return mem[:,0,:]
            elif self.reduce_type == "all_mem":
                return mem
            else:
                raise NotImplementedError(self.reduce_type)
        else:
            return x
    
    @property
    def embedding_size(self):
        return self.attn_layers.dim
...
class XTransformerSeqEncoder(SeqEncoderContainer):
    ...
    def __init__(self,
        ...
        self.col_segm = col_segm

    def forward(self, x):
        time = x.payload[self.col_time]
        ...
        x = self.seq_encoder(x, mask = mask, 
            time = time if self.pass_time_to_encoder else None,
            segm = x.payload[self.col_segm] if self.col_segm else None)
        return x
